...
def main():
    # Makes a pretty banner <3
    ascii_banner = pyfiglet.figlet_format("Favicon Checker")
    print(ascii_banner)
    print("=========================================================================\n")

    if len(sys.argv) != 2:
        print("Usage is: 'python3 favicon_checker.py [favicon url]'")
    else:
        print("Checking URL:     ", sys.argv[1])
...
def check_url(url:str):
    ...
    if r.ok:
        hashed_favicon = hashlib.md5(r.content).hexdigest()

        # Match the favicon we download with one in the database
        if hashed_favicon in FAVICON_DB:
            print("Found Match:      ", FAVICON_DB.get(hashed_favicon))
        else:
            print("No result found compared to list in db")
            print("Favicon hash was: ", hashed_favicon)

    else:
        print("Error with url. Got status code:", r.status_code)
